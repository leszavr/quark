...
function shouldIgnore(path) {
    ...
    return IGNORE_PATTERNS.some(pattern => parts.some(part => pattern.test(part)));
}
/**
 ...
 */
async function scanDirectory(dir, rootPath, depth = 0) {
    const issues = [];
 ...
 */
async function validateStructure(rootPath) {
    const issues = [];
    const requiredDirs = [
        'services',
        'infra',
