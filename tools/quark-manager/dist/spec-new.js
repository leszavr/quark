...
async function runSpecNew(rootDir, serviceName) {
    if (!serviceName) {
    ...
    if (existsSync(specsDir)) {
        const specDirs = await import('node:fs').then(fs => fs.readdirSync(specsDir).filter(f => fs.statSync(resolve(specsDir, f)).isDirectory()));
...
program.parse();
//# sourceMappingURL=spec-new.js.map