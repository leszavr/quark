 ...
 */
async function scanDirectory(dir, root, maxDepth = 5) {
    if (maxDepth <= 0)
        return [];
    const files = [];
 ...
 */
function checkRootFiles(files, root) {
    const issues = [];
 ...
 */
function suggestCorrectLocation(file, root) {
 ...
 */
async function checkImports(file, root) {
    const issues = [];
 ...
 */
function convertToAlias(importPath) {
 ...
 */
function displayIssues(issues, verbose) {
    if (issues.length === 0) {
...
program.parse();
//# sourceMappingURL=check-structure.js.map