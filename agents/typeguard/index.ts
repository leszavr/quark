...
const fix = args.has('--fix') || !dryRun;
const verbose = args.has('--verbose');
const aiAssist = args.has('--ai-assist');
...
for (const typeDep of typeDeps) {
  ...
  for (const dep of skipped) {
    info(`Skipped: ${dep} (used in ${usedTypes[dep]} files)`);
  }
  if (error) {
    info(`ERROR — ${toRemove.length} attempted, restore complete.`);
  } else {
    info(`OK — ${toRemove.length} dependency cleaned, 0 errors.`);
  }
